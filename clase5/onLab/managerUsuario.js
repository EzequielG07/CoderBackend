...
class ManagerUsuario {
  ...
  validarUsuario = async (username, contrasena) => {
    ...
    if (!user) {
      console.log("Usuario o Contrasena no validas"); //! siempre colocar de esta manera y no especificar donde esta el error por seguridad
    } else {
...
const prueba = async () => {
  ...
  // await manager.validarUsuario("labello", "abcde");
};

prueba();

//node managerUsuario.js
